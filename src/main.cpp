...
uint256 hashGenesisBlock("0x56976ec578752667ee37a6e21b37a944ec5b17b2bfc11e97e0882aedc4e4f3c9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Astrocoin: starting difficulty is 1 / 2^12
...
{
    // Astrocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1.0 * 24 * 60 * 60; // Astrocoin: 1.0 days
static const int64 nTargetSpacing = 3.0 * 60; // Astrocoin: 3.0 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Astrocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
    ...
    if (BlockLastSolved == NULL || BlockLastSolved->nHeight == 0 || (uint64)BlockLastSolved->nHeight < PastBlocksMin) { return bnProofOfWorkLimit.GetCompact(); }
        ...
        for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
                ...
                EventHorizonDeviation                        = 1 + (0.7084 * pow((double(PastBlocksMass)/double(180)), -1.228));
...
{
        static const int64        BlocksTargetSpacing                        = 3 * 60; // 180 seconds
        unsigned int                TimeDaySeconds                                = 60 * 60 * 24;
        int64                       PastSecondsMin                                = TimeDaySeconds * 0.38;
        int64                       PastSecondsMax                                = TimeDaySeconds * 1.13;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                ...
                if (pindexLast->nHeight+1 >= 17600) { DiffMode = 2; }
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Astrocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Astrocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1389066606;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2024374987;

        if (fTestNet)
        {
            block.nTime    = 1389066602;
            block.nNonce   = 385270583;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x4a324a4688bbf15548815a8b94393a8a5ffdc102a5de0ce9573c79b2d5a32313"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AstrocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AstrocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AstrocoinMiner(CWallet *pwallet)
{
    printf("AstrocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("astrocoin-miner");
    ...
    try { loop {
        ...
        printf("Running AstrocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AstrocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AstrocoinMiner, pwallet));
}
